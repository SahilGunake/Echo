...
import com.example.echo.fragments.SongPlayingFragment.Statified.activity
...
import com.example.echo.fragments.SongPlayingFragment.Statified.fastforwardImageButton
...
import com.example.echo.fragments.SongPlayingFragment.Statified.rewindImageButton
...
import com.example.echo.fragments.SongPlayingFragment.Statified.songArtist
...
import com.example.echo.models.Songs
...
class SongPlayingFragment : Fragment() {
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        menu?.let { super.onPrepareOptionsMenu(it) }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        if (fromBottomBar != null) {
            ...
            try {
                activity?.let { mediaPlayer?.setDataSource(it, Uri.parse(path)) }
                mediaPlayer?.prepare()
            } catch (e: Exception) {
        ...
        fun on_song_complete() {
            if (!(currentSongHelper.isShuffle)) {
                if (currentSongHelper.isLoop) {
                    ...
                    mediaPlayer?.reset()
                    try {
                        activity?.let { mediaPlayer?.setDataSource(it, Uri.parse(nextSong.songData)) }
                        mediaPlayer?.prepare()
                        mediaPlayer?.start()
